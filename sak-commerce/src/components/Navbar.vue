            ...
            <DialogPanel class="relative flex w-full max-w-xs flex-col overflow-y-auto bg-white pb-12 shadow-xl">
                    ...
                    <div class="grid grid-cols-2 gap-x-4">
                      <div v-for="item in category.featured" :key="item.name" class="group relative text-sm">
                        <div class="aspect-h-1 aspect-w-1 overflow-hidden rounded-lg bg-gray-100 group-hover:opacity-75">
                          <img :src="getImageUrl(item.imageSrc)" :alt="item.imageAlt" class="object-cover object-center" />
                        </div>
                    ...
                    <div v-for="section in category.sections" :key="section.name">
                      ...
                      <ul role="list" :aria-labelledby="`${category.id}-${section.id}-heading-mobile`" class="mt-6 flex flex-col space-y-6">
                        <li v-for="item in section.items" :key="item.name" class="flow-root">
                          <router-link :to="item.href" class="-m-2 block p-2 text-gray-500">{{ item.name }}</router-link>
                        </li>
                      </ul>
                    </div>
                  </TabPanel>
              ...
              <div class="space-y-6 border-t border-gray-200 px-4 py-6">
                 <div class="flow-root" v-if="!isAuthenticated">
                   <router-link to="/login" class="-m-2 block p-2 font-medium text-gray-900">Login</router-link>
                 </div>
                 <div class="flow-root" v-if="!isAuthenticated">
                   <router-link to="/register" class="-m-2 block p-2 font-medium text-gray-900">Register</router-link>
                 </div>
                 <div class="flow-root" v-if="isAuthenticated && isAdmin">
                   <router-link to="/admin" class="-m-2 block p-2 font-medium text-gray-900">Admin Panel</router-link>
                 </div>
                 <div class="flow-root" v-if="isAuthenticated">
                   <a href="#" @click.prevent="handleLogout" class="-m-2 block p-2 font-medium text-gray-900">Logout</a>
                 </div>
              </div>
            </DialogPanel>
          </TransitionChild>
        ...
        <div class="flex h-16 items-center">
          ...
          <div class="ml-4 flex lg:ml-0">
            <router-link to="/">
              ...
              <img class="h-8 w-auto" src="/alexis_icon.png" alt="" />
            </router-link>
          </div>
                            ...
                            <div v-for="section in category.sections" :key="section.name">
                              ...
                              <ul role="list" :aria-labelledby="`${section.name}-heading`" class="mt-6 space-y-6 sm:mt-4 sm:space-y-4">
                                <li v-for="item in section.items" :key="item.name" class="flex">
                                  <router-link :to="item.href" class="hover:text-gray-800">{{ item.name }}</router-link>
                                </li>
                              </ul>
                            </div>
                          </div>
          ...
          <div class="ml-auto flex items-center">
            <div class="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
              <div v-if="isAuthenticated" class="relative ml-3">
                 ...
                 <a href="#" @click.prevent="handleLogout" class="ml-4 text-sm font-medium text-gray-700 hover:text-gray-800">Logout</a>
              </div>
              <template v-else>
                <router-link to="/login" class="text-sm font-medium text-gray-700 hover:text-gray-800">Login</router-link>
                <span class="h-6 w-px bg-gray-200" aria-hidden="true" />
                <router-link to="/register" class="text-sm font-medium text-gray-700 hover:text-gray-800">Register</router-link>
              </template>
            </div>
            ...
            <div class="ml-4 flow-root lg:ml-6">
              <a href="#" @click.prevent="isCartOpen = true" class="group -m-2 flex items-center p-2">
  ...
  categories: [
    {
      ...
      featured: [
        { 
          ...
          href: '/product/4',
          imageSrc: products.find(p => p.id === 4).image,
          imageAlt: 'Axis Noise-Cancelling Headphones',
        },
        { 
          name: 'Axis Bluetooth Speaker',
          href: '/product/2',
          imageSrc: products.find(p => p.id === 2).image,
          imageAlt: 'Axis Bluetooth Speaker',
        },
      ],
      sections: [
        {
          id: 'audio-products',
          name: 'Audio Products',
          items: products.filter(p => p.category === 'Audio').map(p => ({ name: p.name, href: `/product/${p.id}`}))
        },
      ],
    },
    {
      ...
      featured: [
        { 
          ...
          href: '/product/1',
          imageSrc: products.find(p => p.id === 1).image,
          imageAlt: 'Axis Wireless Charger',
        },
      ],
      sections: [
        {
          id: 'carga-products',
          name: 'Carga Products',
          items: products.filter(p => p.category === 'Carga').map(p => ({ name: p.name, href: `/product/${p.id}`}))
        },
      ],
    },
    {
      ...
      featured: [
        { 
          ...
          href: '/product/3',
          imageSrc: products.find(p => p.id === 3).image,
          imageAlt: 'Axis Smartwatch',
        },
      ],
      sections: [
        {
          id: 'accesorios-products',
          name: 'Accesorios Products',
          items: products.filter(p => p.category === 'Accesorios').map(p => ({ name: p.name, href: `/product/${p.id}`}))
        },
      ],
    },
  ],
